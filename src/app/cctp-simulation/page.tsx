...
export default function CCTPSimulationPage() {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      toast.success(`成功從 ${selectedChain === 'base' ? 'Base Sepolia' : selectedChain === 'avalanche' ? 'Avalanche Fuji' : 'Ethereum Sepolia'} 提取資金!`, {
        duration: 5000,
      });
    } catch (error) {
      toast.error("處理過程中發生錯誤，請重試");
      console.error("跨鏈操作錯誤:", error);
    } finally {
  ...
  if (!isClient) {
    return <div className="cctp-container">
      <div className="cctp-header">
        <h1 className="cctp-title">FlashDAO跨鏈資金提取</h1>
        <p className="cctp-subtitle">載入中...</p>
      </div>
    </div>;
  }
    ...
    <div className="cctp-container">
      <div className="cctp-header">
        ...
        <p className="cctp-subtitle">
          恭喜 <span className="winner-name">{winningCandidate.name}</span> 成為得票最高的志願者！
          <br />您可以選擇從以下任一鏈上提取全部援助資金。
        </p>
      </div>
      ...
      <div className="cctp-card">
        <div className="card-header">
          <div className="card-title-container">
            ...
            <h2 className="card-title">跨鏈資金提取</h2>
          </div>
          <p className="card-description">
            使用 Circle CCTP v2 技術，您可以選擇將資金轉移到任何支持的區塊鏈上
          </p>
        </div>
        ...
        <div className="card-content">
          <div className="fund-info">
            <div className="fund-amount">
              <span className="label">總獎金金額</span>
              <span className="amount">{totalFund.toLocaleString()} USDC</span>
            </div>
            <div className="wallet-info">
              <span className="label">接收錢包</span>
          ...
          {!isCompleted ? (
            <>
              <div className="chain-selection">
                <h3 className="selection-title">選擇提取到的目標鏈</h3>
              ...
              >
                {isProcessing ? (
                  ...
                  <>提取資金到 {selectedChain === 'base' ? 'Base Sepolia' : selectedChain === 'avalanche' ? 'Avalanche Fuji' : 'Ethereum Sepolia'}</>
                )}
              </Button>
            </>
          ) : (
            <div className="success-container">
              ...
              <h3 className="success-title">資金提取成功！</h3>
              <p className="success-message">
                您已成功將 {totalFund.toLocaleString()} USDC 提取到 {selectedChain === 'base' ? 'Base Sepolia' : selectedChain === 'avalanche' ? 'Avalanche Fuji' : 'Ethereum Sepolia'} 鏈上。
              </p>
              <div className="transaction-details">
                <p className="tx-info">交易哈希:</p>
